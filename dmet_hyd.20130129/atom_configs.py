...
def GetAtomAverageOccupation(iElement, iShell, iAngMom):
   """get atomic average occupation for AOs for density guess purposes:
   iElement: 1-based element number (e.g., 6 == Carbon),
   iShell: 1-based shell number (e.g., for Carbon 2p (the valence orb), give 2)
   iAngMom: 0-based angular momentum (e.g., s == 0, p == 1)"""
   iShell = iShell - iAngMom - 1 # convert 1-based input to 0-based offset.
   if ( iShell < 0 ):
      return 0.  # no 1p orbital.
   iRange = g_AtomicOccupationRanges[iElement-1]
   ShellOccs = g_AtomicOccupations[iRange[0]:iRange[1]]
   #print "ShellOccs:", ShellOccs
   for iAm_ in range(iAngMom):
      if ( len(ShellOccs) == 0 ):
         return 0.
      iNull = ShellOccs.index(0)
      ShellOccs = ShellOccs[iNull+1:]
   if len(ShellOccs) == 0:
      return 0.
   iNull = ShellOccs.index(0)
   ShellOccs = ShellOccs[:iNull]
   #print "ShellOccs for current AM:", ShellOccs
   if iShell >= len(ShellOccs):
      return 0. # polarization shell
   #print "iShell = ", iShell
   return ShellOccs[iShell]/(2.*iAngMom + 1.)

if ( __name__ == "__main__" ):
   iElement = 1
   for (iShell, iAngMom) in [ (1,0), (2,0), (2,1) ]:
      print "Element: %i   AO: %i%s   fAvgOcc: %.3f" % (iElement, iShell, "spdfghj"[iAngMom], GetAtomAverageOccupation(iElement, iShell, iAngMom))
   iElement = 6
   for (iShell, iAngMom) in [ (1,0), (2,0), (2,1) ]:
      print "Element: %i   AO: %i%s   fAvgOcc: %.3f" % (iElement, iShell, "spdfghj"[iAngMom], GetAtomAverageOccupation(iElement, iShell, iAngMom))
