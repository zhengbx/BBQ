...
def resmin(fn, x0, Algo="Hard"):
    def fn_value(x):
       r = fn(x)
       return dot(r,r)
    #return scipy.optimize.fmin_bfgs(fn_value,x0,fn,gtol=1e-12,disp=0)
    #x0 = scipy.optimize.anneal(fn_value,x0,T0=0.01,schedule='cauchy')[0]
    #print x0
    #print fn(x0)
    #raise SystemExit
    #return r
    ...
    if 1:
        ...
        #for it in range(20):
           ...
           #def fn_value(x):
              #r = fn(x)
              #return dot(r,r)
           #x = scipy.optimize.fmin_bfgs(fn_value,x,fn,gtol=1e-12,disp=0)
           #print "WARNING: resmin failed to converge. Final gradient after resmin: %8.2e  after fmin_bfgs: %8.2e" % (norm(r),norm(fn(x)))
        return x
#-6.17572748
