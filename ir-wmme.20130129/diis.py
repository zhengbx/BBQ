...
class FDiisContext:
    ...
    def RemoveBadVectors(self,iThis):
        ...
        B0 = dot(transpose(Errs1),Errs1)
        EMin = min(diag(B0))

        iVecs = []
        for i in range(nDim):
            if ( B0[i,i] <= 1e12 * EMin or i == iThis ):
                iVecs.append(i)
        if ( len(iVecs) != nDim ):
            iVecs = array(iVecs)
            nDim = len(iVecs)
    ...
    def Apply(self, T_, R_, O_ = None, Skip=None):
        ...
        if ( dot(R,R) < 1e-30 ): # <- otherwise divide by zero in the B scaling.
        ...
        if ( self.nDim == 0 ):
            self.Amps = zeros((len(T), self.MaxDim))
            self.Errs = zeros((len(R), self.MaxDim))
            if (O is not None):
                self.Othr = zeros((len(O), self.MaxDim))
            else:
                self.Othr = None
